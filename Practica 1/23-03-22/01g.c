...
#include <time.h>
// #include <conio.h>      /** Solo para WINDOWS, LINUX Y MAC es icnompatible.*/
...
void cargarArrC2(char arrS[TAM]);
...
void caso01(){
	...
	cargarArrC2(nombre);                /** Carga arreglo desde teclado*/
...
void cargarArrC(char arrC[TAM]){
	...
	char term='\r';                 /** utilizar \r cuando utiliza getche*/
	                                /** \\r es la tecla ENTER, no confundir con \\n Que representa una nueva linea*/
    if(i<TAM-1)
	...
	ES COMPATIBLE CON WINDOWS MAC LINUX
	
	Falta confirmación de pruebas 25/03/2021
	2021/05/25 Probado en MAC 10.4 "Mojave". Delfina Las Heras
	2021/05/25 Probado en MAC 11.2 "Big Sur". Carolina Suarez
	
	
	**************************/
	
void cargarArrC2(char arrC[TAM]){
	int i=0;
	char aux;
	char term='\n';                  /** utilizar \n con getchar */
	printf("Ingrese contenido: ");
	...
	while(aux!=term && i<TAM-1){     /** usar el terminador correspondiente a cada sistema operativo*/
		                             /** Se debe controlar no pasasrse del tamanio TAM*/
		arrC[i] = aux;               /** Asignar caracter al arreglo arrC en la posición i*/
		i++;                         /** cambia posicion del arreglo / es variable de control */
		if(i<TAM-1)
